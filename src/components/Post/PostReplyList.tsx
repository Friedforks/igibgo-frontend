...
const ReplyItem: React.FC<ReplyItemProps> = ({reply, depth}) => {
    ...
    const onLoadChildReplies = async (parentId: number) => {
        return new Promise<PostReply[]>((resolve, reject) => {
            axiosInstance.get("/forum/reply/child", {
                params: {
    ...
    const onLike = () => {
        axiosInstance.post("/forum/like/reply", 0, {
            params: {
            ...
            if (response.data.code == ResponseCodes.SUCCESS) {
                ...
                console.log("Error in like request: " + response.data.message);
            }
        }).catch((error) => {
    ...
    const onDelete = () => {
        axiosInstance.delete("/forum/reply/delete", {
            params: {
                postReplyId: reply.postReplyId,
                token: localStorage.getItem("token")
            }
            ...
            if (response.data.code == ResponseCodes.SUCCESS) {
                ...
                console.log("Error in delete request: " + response.data.message);
            }
        }).catch((error) => {
    ...
    const onReply = () => {
        ...
        axiosInstance.post("/forum/reply/new", formData, {
            ...
            if (response.data.code == ResponseCodes.SUCCESS) {
                ...
                console.log("Error in reply request: " + response.data.message);
            }
        }).catch((error) => {
                    ...
                    secondary={
                        <>
                            <Typography
                                ...
                                color="text.primary"
                            >
                                <MdPreview content={reply.replyContent}
                                           maxWidth={window.innerWidth * 0.9 - 56 - (depth * 32)}/>
                            </Typography>
                            <Box sx={{mt: 1}}>
                                <Button
                                    startIcon={<ThumbUp/>}
                                ...
                                {reply.childReplyCount > 0 && childReplies.length === 0 && (
                                    <Button
                                        size="small"
                                        onClick={handleLoadChildReplies}
                                        disabled={isLoadingChildReplies}
                                    >
                                        {isLoadingChildReplies ? 'Loading...' : `Load ${reply.childReplyCount} 
                                         ${reply.childReplyCount > 1 ? "Replies" : "Reply"}`}
                                    </Button>
                                )}
                            </Box>
                            ...
                            {isReplying && (
                                <Box sx={{mt: 2}}>
                                    <MdEditor ref={editorRef} maxWidth={window.innerWidth * 0.9 - 56 - depth * 32}
